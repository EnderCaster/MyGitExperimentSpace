...
for dtype in [np.int8,np.int32,np.int64]:
    print("min of {}: ".format(dtype), np.iinfo(dtype).min)
    print("max of {}: ".format(dtype), np.iinfo(dtype).max)
for dtype in [np.float32,np.float64]:
    print("min of {}: ".format(dtype), np.finfo(dtype).min)
    print("max of {}: ".format(dtype), np.finfo(dtype).max)
...
print("sum count : ",np.isnan(Z).sum())
print("index: ", np.where(np.isnan(Z)))
...
from PIL import Image
import requests
URL='http://static.shiyanlou.com/img/logo-black.png'
response = requests.get(URL)
I=Image.open(BytesIO(response.content))
shiyanlou=np.asarray(I)
shiyanlou
